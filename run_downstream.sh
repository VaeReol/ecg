...
Options:
  ...
  --encoder_path PATH             Path of encoder checkpoint (optional)
  -h, --help                      Print help
EOM
...
function set_options()
{
    arguments=$(getopt --options f:h \
                       --longoptions master_port:,gpus:,config_path:,output_dir:,exp_name:,resume:,start_epoch:,encoder_path:,help \
                       --name $(basename $0) \
                       -- "$@")
...
echo -e "\tENCODER_PATH: ${ENCODER_PATH}"
...
if [ $NUM_GPUS -gt 1 ]; then
    torchrun $TORCHRUN_ARGS main_downstream.py $MAIN_ARGS
else
    python main_downstream.py $MAIN_ARGS
fi
