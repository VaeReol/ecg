...
def get_parser():
    ...
    parser.add_argument('-i',
                        '--input_dir',
                        type=str,
                        default='/home/data/mimic-iv-ecg-diagnostic-electrocardiogram-matched-subset-1.0/files',
                        help="Path to the WFDB ECG database directory.")
    parser.add_argument('-o',
                        '--output_dir',
                        type=str,
                        default='/ssd1/stmem/train_data_id/ecg',
                        help="Path to the directory where the preprocessed signals will be saved.")
    parser.add_argument('--index_path',
                        type=str,
                        default="/ssd1/stmem/train_data_id/index.csv",
...
def run(args):
    ...
    for record_rel_path in tqdm(record_rel_paths):
        ...
        for idx, cropped_signal in enumerate(cropped_signals):
            ...
            # 累积索引数据
            index_data.append([f"{record_rel_path}_{idx}.pkl",
            ...
            # 每 10,000 条数据写入一次，减少 I/O 操作
            if num_saved % 10000 == 0:
                index_df = pd.DataFrame(index_data, columns=["RELATIVE_FILE_PATH", "FILE_NAME", "SAMPLE_RATE", "SUBJECT_ID", "SOURCE"])
                index_df.to_csv(args.index_path, mode='a', index=False, header=not os.path.exists(args.index_path))
                index_data = []  # 清空缓存数据
