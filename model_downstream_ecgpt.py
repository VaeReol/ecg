...
class EEGTransformerPredictor_old(nn.Module):
    """ EEG Transformer """
    def __init__(
        self,
        num_patches,
    ...
    def forward(self, x, mask_x=None, mask_t=None): # bs, mN, embed_num, D
...
class Downstream_model(nn.Module):
    def __init__(self, load_path="/ssd1/qinzehao/0728/pretrain/checkpoint-60.pth"):
        super().__init__()    
        self.chans_num = 12
        ...
        predictor = EEGTransformerPredictor_old(
            img_size=[12, 2250], # 这里在跑单导联/6导联实验的时候需要修改
            patch_size=75,
            num_patches=(12, 30),
            embed_num=1,
        ...
        # del checkpoint_model['predictor.time_embed.freqs']
        # 去掉 'encoder.' 前缀
        ...
        # 去掉 'predictor.' 前缀
        adjusted_checkpoint_model_predictor = {}
        for k, v in checkpoint_model.items():
            if k.startswith('predictor.'):
                new_key = k[len('predictor.'):]
                adjusted_checkpoint_model_predictor[new_key] = v
            else:
                adjusted_checkpoint_model_predictor[k] = v
        state_dict_predictor = self.predictor.state_dict()
        for k in ['head.weight', 'head.bias']:
            if k in adjusted_checkpoint_model_predictor and adjusted_checkpoint_model_predictor[k].shape != state_dict_predictor[k].shape:
                print(f"Remove key {k} from pre-trained checkpoint")
                del adjusted_checkpoint_model_predictor[k]

        self.decoder_dim = 256
        # 加载调整后的权重
        msg = self.target_encoder.load_state_dict(adjusted_checkpoint_model, strict=False)
        msg_predictor = self.predictor.load_state_dict(adjusted_checkpoint_model_predictor, strict=False)
        print("Missing keys in target_encoder:", msg.missing_keys)
        print("Missing keys in predictor:", msg_predictor.missing_keys)
        self.norm = nn.LayerNorm(256)
        self.head = nn.Linear(256, 5)
        # self.decoder         = torch.nn.TransformerDecoder(
        #                             decoder_layer=torch.nn.TransformerDecoderLayer(self.decoder_dim, 4, self.decoder_dim*4, activation=torch.nn.functional.gelu, batch_first=False),
        #                             num_layers=8
        #                         )
        # self.cls_token =        torch.nn.Parameter(torch.rand(1,1,self.decoder_dim)*0.001, requires_grad=True)
        self.chan_conv       = Conv1dWithConstraint(12, self.chans_num, 1, max_norm=1)
        self.linear_probe1   =   LinearWithConstraint(256, self.decoder_dim, max_norm=1)
        self.linear_probe2   =   LinearWithConstraint(self.decoder_dim, 5, max_norm=0.25)
        ...
        # self.linear1   =  nn.Linear(256, decoder_dim)
        # self.linear2   =  nn.Linear(decoder_dim, 5)
    ...
    def forward(self, x):
        ...
        # h = z.flatten(2)
        # h = self.linear_probe1(self.drop(h))
        # pos = create_1d_absolute_sin_cos_embedding(h.shape[1], dim=self.decoder_dim)
        # h = h + pos.repeat((h.shape[0], 1, 1)).to(h)
        # h = torch.cat([self.cls_token.repeat((h.shape[0], 1, 1)).to(h.device), h], dim=1)
        # h = h.transpose(0,1)
        # h = self.decoder(h, h)[0,:,:]
        # pred = self.linear_probe2(h)

        predictor_output = self.predictor(z)
        pred_latent = torch.mean(predictor_output, dim=(1, 2))
        pred_latent = self.norm(pred_latent)
        # print(z.shape)
        pred = self.head(pred_latent)
        return pred
...
if __name__ == '__main__':
    ...
    result = model(x)
    pretrain_ckpt = torch.load("/ssd1/qinzehao/new/v5/pretrain/checkpoint-50.pth", map_location='cpu')
    checkpoint_model = pretrain_ckpt['model']
    del checkpoint_model['predictor.time_embed.freqs']
    print(checkpoint_model.keys())
    # base 0.926
    # no pos 0.922
    # head/layer 4->2  0.925