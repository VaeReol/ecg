...
import random
import copy
from model_pretraining_v4 import EEGTransformer, EEGTransformerPredictor, EEGTransformerReconstructor, apply_mask, ECGProjection
from configs import *
...
class FreRA(Module):
    def __init__(self, len_sw, fs, fmin=0.65, fmax=40, device=None, dtype=None):
        super(FreRA, self).__init__()
        print(f'Initializing FreRA with freq range {fmin}-{fmax} Hz')
        ...
        n_fourier_comp = len_sw // 2 + 1  # rfft 输出长度

        # 频率掩码
        freqs = torch.fft.rfftfreq(len_sw, d=1/fs)
        self.register_buffer('freq_mask', (freqs >= fmin) & (freqs <= fmax))
    ...
    def forward(self, x, temperature=0.1):
        ...
        # 只在目标频率范围内增强
        para = para * self.freq_mask.float().unsqueeze(-1) + (~self.freq_mask).float().unsqueeze(-1)
        ...
        x_ft = x_ft * torch.unsqueeze(para[:, 0] + noise_para[:, 0] * scaling_factor, 0)
        aug = torch.fft.irfft(x_ft, n=x.shape[-1], dim=-1)

        return aug


# ------------------ ECG 多导联增强封装 ------------------
class ECG_FreRA(Module):
    def __init__(self, num_leads, len_sw, fs, fmin=0.65, fmax=40, device=None):
        super(ECG_FreRA, self).__init__()
        self.num_leads = num_leads
        self.lead_aug = nn.ModuleList([
            FreRA(len_sw, fs, fmin, fmax, device=device) for _ in range(num_leads)
        ])

    def forward(self, x, temperature=0.1):
        # x: (B, num_leads, length)
        out = []
        for i in range(self.num_leads):
            out.append(self.lead_aug[i](x[:, i, :], temperature))
        return torch.stack(out, dim=1)  # (B, num_leads, length)
...
class LitEEGPT(nn.Module):
    ...
    def forward_context(self, x, x_aug, mask_x, mask_y):
        z = self.encoder(x, self.chans_id.to(x), mask_x=mask_x)
        z_aug = self.encoder(x_aug, self.chans_id.to(x_aug), mask_x=mask_x)
        z, comb_z = self.predictor(z, mask_x=mask_x)
        z_aug, comb_z_aug = self.predictor(z_aug, mask_x=mask_x)
    ...
    def forward(self, x):
        ...
        x_aug = x.clone()
        # print(x.shape)
        self.fft_aug = self.fft_aug.to(x.device)
        x_aug = self.fft_aug(x_aug)
        mask_x, mask_y = self.make_masks(self.encoder.num_patches)
        h, y = self.forward_target(x_aug, mask_y)
        z, r = self.forward_context(x, x_aug, mask_x, mask_y)
