    ...
    for data_iter_step, samples in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        for name, param in model.named_parameters():
            if param.requires_grad and param.grad is None:
                print(f"[Unused Grad] {name} has no gradient.")

        # check_unused_parameters(model)
        ...
        if log_writer is not None and (data_iter_step + 1) % accum_iter == 0:
            ...
            log_writer.add_scalar('pretrain_loss_constra', loss_1, epoch_1000x)
            log_writer.add_scalar('pretrain_loss_recon', loss_2, epoch_1000x)
        ...
        with torch.no_grad():
            m = next(momentum_scheduler)
            for param_q, param_k in zip(model.module.projection.parameters(), model.module.target_projection.parameters()):
                param_k.data.mul_(m).add_((1.0 - m) * param_q.detach().data)
