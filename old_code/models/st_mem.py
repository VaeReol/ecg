...
class ST_MEM(nn.Module):
    def __init__(self,
        ...
        self.num_patches = seq_len // patch_size # 2250 / 75 = 30
    ...
    def random_masking(self, x, mask_ratio):
        ...
        return x_masked, mask, ids_restore # mask是0和1的矩阵 (bs, num_leads, num_patch) 0表示留下来 1表示被mask
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        """
        # embed patches
        x = self.to_patch_embedding(x) # (bs, 12, 30, 768)
        b, _, n, _ = x.shape
        ...
        return x, mask, ids_restore # （bs, num_leads * （keep_len + 2）, embed_dim)  (bs, num_leads, num_patch)  (bs, num_leads, num_patch)

    def forward_decoder(self, x, ids_restore):
        x = self.to_decoder_embedding(x)  # （bs, num_leads * （keep_len + 2）, 256) 
