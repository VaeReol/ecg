...
class ST_MEM_ViT(nn.Module):
    def __init__(self,
        ...
        # transformer layers
        drop_path_rate_list = [x.item() for x in torch.linspace(0, drop_path_rate, depth)] # 从0到drop_path_rate生成均匀分布的depth个数
    ...
    def forward_encoding(self, series):
        ...
        b, _, n, _ = x.shape # x.shape: (bs, num_leads, num_patch, patch_embedding_size)
        ...
        x = torch.mean(x, dim=(1, 2)) # (bs, patch_embedding_size)
        return self.norm(x)
