...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=(12, 2500), patch_size=16, patch_stride=None, embed_dim=768):
        super().__init__()
...
class EEGTransformerReconstructor(nn.Module):
    ...
    def forward(self, x, chan_ids=None, mask_1=None, mask_x=None, mask_y=None):
        ...
        if mask_x is not None:
            ...
            freqs_x      = self.time_embed.prepare_freqs((eN, mN), x.device, x.dtype) # NC, time_dim
        ...
        if mask_y is not None:
            ...
            mask_1      = torch.floor(mask_1[:,0] / C).long()
            mask_id          = torch.concat([mask_1.to(x.device), mask_y.to(x.device)], dim=0)          
            x                = torch.index_select(x, dim=1, index=torch.argsort(mask_id))  
            freqs_x                = torch.index_select(freqs_x, dim=0, index=torch.argsort(mask_id))
...
class EEGTransformerPredictor(nn.Module):
    ...
    def forward(self, x, mask_x=None, mask_t=None): # bs, mN, embed_num, D
        ...
        # 因为重建latent域和重建原始信号的任务不同，上述的x经过了transformer，所以这里要单独和未经过transformer的输入进行concat
        # 然后送到后面进行重建原始信号 避免不同任务互相影响
        if self.use_part_pred and mask_x is not None:
            cmb_x = torch.index_select(x, dim=1, index=mask_t.to(x.device)) # 需要预测的token
            cmb_x = torch.concat([inp_x, cmb_x], dim=1) 
            cmb_x = torch.index_select(cmb_x, dim=1, index=torch.argsort(mask_id)) # 恢复到正确的顺序（送入后面重建原始信号）
            return x, inp_x
        return x
...
class ECGProjection(nn.Module):
    def __init__(
        ...
        self.projection_head = nn.Sequential(
            nn.Linear(256, 1024),  # 初始定义时可以用一个默认的 input_dim
            nn.ReLU(),
