...
print(os.getcwd())
import sys
sys.path.append('/home/zehaoqin/ST-MEM')
...
class ST_MEM(nn.Module):
    ...
    def forward(self, x, mask_x=None, mask_t=None):
        ...
        x = x.permute(0, 2, 1, 3) # (bs, 30, 12, 768)
        B, N, C, D = x.shape
        assert N==30 and C==12
        # add positional embeddings

        # 2) lead embeddings
        n_masked_with_sep = x.shape[1]
        ...
        if mask_x is not None:
            ...
            x = self.apply_mask(mask_x, x)# (bs, mn*N, mc*C, D)
            B, N, C, D = x.shape
        x = x.flatten(0, 1)
        ...
        if mask_t is not None:
            ...
            x = self.apply_mask_t(mask_t, x)# B, mN, D        
        
        x = x.reshape((B, N, 1, -1))

        return x # （bs, num_leads * （keep_len + 2）, embed_dim)  (bs, num_leads, num_patch)  (bs, num_leads, num_patch)

    def forward_target(self, x, mask_y):
        with torch.no_grad():
            h = self.target_encoder(x, self.chans_id.to(x)) # B, N, embed_num, D
            h = F.layer_norm(h, (h.size(-1),))
            C, N = self.encoder.num_patches # C=58 N=16
            ...
            y = self.apply_mask(mask_y.to(x.device), x)
            if self.USE_LN:
...
if __name__ == "__main__":
    ...
    model = st_mem_vit_base_dec256d4b()
    print('开始创建mask')
    mask_x, mask_y = model.make_masks((12, 30))
    print('mask创建完毕')
    z = model.forward_encoder(x, mask_x)
    print(z.shape)
    predictor = EEGTransformerPredictor(num_patches=(12, 30))
    z, comb_z = predictor(z, mask_x=mask_x)
    print(z.shape)
    print(comb_z.shape)