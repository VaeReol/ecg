...
from model_downstream import Downstream_model
...
def main(config):
    ...
    def print_class_distribution(dataset, num_classes=9, name="训练集"):
        """
        打印数据集中各类别的样本数量及比例
        
        Args:
            dataset: 数据集对象，需包含labels属性或可通过__getitem__获取标签
            num_classes: 总类别数，若为None则自动推断
        """
        # 检查数据集是否有labels属性
        if hasattr(dataset, 'labels') and dataset.labels is not None:
            labels = np.array(dataset.labels)
        else:
            # 若没有直接存储的labels，需要遍历数据集获取
            print("数据集未直接存储labels，需要遍历获取，这可能需要一些时间...")
            labels = []
            for i in range(len(dataset)):
                # 根据__getitem__的返回值判断：若返回元组，则第二个元素为标签
                item = dataset[i]
                if isinstance(item, (tuple, list)) and len(item) > 1:
                    labels.append(item[1].item() if torch.is_tensor(item[1]) else item[1])
                else:
                    raise ValueError("无法从数据集中获取标签，请确保__getitem__返回包含标签的元组")
        
        # 统计各类别数量
        unique_classes, counts = np.unique(labels, return_counts=True)
        
        # 若指定了总类别数，补充统计缺失类别的数量
        if num_classes is not None:
            all_classes = np.arange(num_classes)
            missing_classes = set(all_classes) - set(unique_classes)
            for cls in missing_classes:
                unique_classes = np.append(unique_classes, cls)
        ...
        # 打印结果
        print(f"{name}类别分布（总样本数: {total_samples}）:")
        for cls, cnt, prop in zip(unique_classes, counts, proportions):
            print(f"类别 {cls}: {cnt} 个样本，占比 {prop*100:.2f}%")
        
        return unique_classes, counts, proportions

    print_class_distribution(dataset_train, name="训练集")
    
    dataset_valid = build_dataset(config['dataset'], split='valid')
    print_class_distribution(dataset_valid, name="验证集")
    ...
    criterion, output_act = build_loss_fn(config['loss'], device=device)
    best_loss = float('inf')
    ...
    if config.get('test', False) and misc.is_main_process():
        ...
        print_class_distribution(dataset_test, name="测试集")
